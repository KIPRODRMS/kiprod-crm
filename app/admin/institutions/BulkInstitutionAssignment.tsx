...
import { bulkAssignInstitutions } from "./actions";
...
type BulkInstitutionAssignmentProps = {
...
export default function BulkInstitutionAssignment({
  ...
  profiles,
  returnUrl,
}: BulkInstitutionAssignmentProps) {
    ...
    >
      ...
      <div className="border-b border-slate-200 bg-slate-50 p-5">
        <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {selected.size} selected from{" "}
              {institutions.length} visible institutions
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
          ...
          >
            <option value="">
              Unassign selected institutions
            </option>
      ...
      <div className="divide-y divide-slate-200">
        {institutions.map((institution) => {
          const checked = selected.has(institution.id);

          const owner = institution.assigned_to
            ? profileMap.get(institution.assigned_to) ||
              "Unknown Team Member"
            : "Unassigned";
          ...
          return (
            <article
              key={institution.id}
              className={`grid gap-4 px-5 py-4 transition lg:grid-cols-[40px_minmax(220px,1.4fr)_130px_100px_150px_170px_auto] lg:items-center ${
                checked
                  ? "bg-amber-50"
                  : "hover:bg-slate-50"
              }`}
            >
              <label className="flex items-center">
                <input
                  ...
                  checked={checked}
                  onChange={() =>
                    toggleInstitution(institution.id)
                  }
