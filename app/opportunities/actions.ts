...
export async function createOpportunity(
  ...
  if (!institutionId) {
    redirect(
      "/opportunities?error=Please select an institution"
    );
  }

  if (!stageId) {
    redirect(
      "/opportunities?error=Please select a pipeline stage"
    );
  }

  if (!title) {
    redirect(
      "/opportunities?error=Opportunity title is required"
    );
  }
  ...
  if (
    ...
    redirect(
      "/opportunities?error=Enter a valid opportunity value"
    );
  }
  ...
  if (stageError || !stage) {
    redirect(
      "/opportunities?error=Invalid pipeline stage"
    );
  }
  ...
  if (followUpValue) {
    ...
    if (
      ...
      redirect(
        "/opportunities?error=Enter a valid follow-up date"
      );
    }
  ...
  if (error) {
    redirect(
      `/opportunities?error=${encodeURIComponent(
        error.message
      )}`
    );
  }
